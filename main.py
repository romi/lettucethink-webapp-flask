...
def storeImage(bed, zone, label):
    ...
    expid = action[2]
    path = "%s/%s_%s_%s_%s_%s.jpg" % (imgdir, date, farmid, zoneid, expid, label)
    print("Saving topcam image to %s" % (path));
    r = requests.get("http://localhost:10000/image.jpg")
    file = open(path, "wb")
...
def topcam():
    img = requests.get("http://localhost:10000/image.jpg")
    #img = requests.get("http://localhost:10000/image.jpg?w=320&h=240")
